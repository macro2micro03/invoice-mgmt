...
def test_send_email_success():
    ...
    assert kwargs["smtp_email"] == "sender@gmail.com"
...
def test_send_email_reports_auth_failure_as_400():
    with patch("app.routers.email.email_sender.send_email_with_attachment") as mock_send:
        mock_send.side_effect = smtplib.SMTPAuthenticationError(535, b"bad credentials")
        response = _send()

    assert response.status_code == 400
    assert "Gmail 로그인" in response.json()["detail"]


def test_send_email_reports_other_smtp_errors_as_502():
    with patch("app.routers.email.email_sender.send_email_with_attachment") as mock_send:
