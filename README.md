...
- 자동 인식 기준: 페이지 제목에 "철근 납품 확인서"가 포함된 페이지만 갑지로 인식합니다. 이 제목이 없으면 인식하지 못하니, 다른 형식의 반입송장이라면 제목을 확인해주세요.
- 자동 채움: 규격별(철근경) 합산 수량(로스감안중량, Ton), 거래처(공장명/비고란의 제조회사명), 반입일자(갑지 납품일 중 최댓값), 송장번호, 문서번호(자동 증가), 접수일자/검수일자(생성일)
